...
typedef struct {
    ...
    block_t *blocks;    // array of memory blocks returned by malloc when executing
    size_t peak_size;   // total paylaod bytes at peak in-use
} script_t;


static void parse_script(const char *filename, script_t *script);
static bool eval_correctness(script_t *script, size_t *p_used, bool quiet);
...
int main(int argc, char *argv[])
{
    ...
    int nresults = 0, nfailures = 0;
    int total_util = 0;
    for (int i = optind; i < argc; i++) {
        script_t script = { .ops = NULL, .blocks = NULL};
        parse_script(argv[i], &script);
        printf("\nEvaluating allocator on %s... ", script.name);
        size_t used_segment = 0;
        bool valid = eval_correctness(&script, &used_segment, quiet);
        if (valid) {
            printf("successfully serviced %d requests. (payload/segment = %zu/%zu)", script.num_ops, script.peak_size, used_segment);
...
static void parse_script(const char *path, script_t *script)
{
    ...
    for (int i = 0; read_line(buf, sizeof(buf), fp, &lineno) ; i++) {
        ...
        if (!script->ops[i].op || script->ops[i].id < 0 || script->ops[i].size > MAX_REQUEST_SIZE)
...
static bool eval_correctness(script_t *script, size_t *p_used, bool quiet)
{
    ...
    for (int req = 0; req < script->num_ops; req++) {
        ...
        void *p, *newp, *oldp = script->blocks[id].ptr, *last_touched;
        ...
        switch (script->ops[req].op) {
            ...
            case ALLOC:
                ...
                last_touched = (char *)p + requested_size;
                break;
            ...
            case REALLOC:
                ...
                last_touched = (char *)newp + requested_size;
                break;
        ...
        if (!quiet && !validate_heap()) { // check heap consistency after each request
        ...
        if (cur_size > script->peak_size) script->peak_size = cur_size;
        if (last_touched > heap_end) heap_end = last_touched;
    }
    ...
    for (int id = 0;  id < script->num_ids;  id++)
        if (!verify_payload(script->blocks[id].ptr, script->blocks[id].size, id, script, -1, "at exit"))
            return false;
    *p_used = (char *)heap_end - (char *)heap_segment_start();
    return true;
}
